...
// issue #215 — 连接级审批命中但未获批时的两个运行时状态。types.ts 的
// McpConnectionState['status'] 联合不在本次改动范围，故这里用 unknown 窄化——
// 运行时是普通字符串，UI 侧按字符串消费。
const AWAITING_APPROVAL = 'awaiting-approval' as unknown as McpConnectionState['status']
const APPROVAL_DENIED = 'denied' as unknown as McpConnectionState['status']
