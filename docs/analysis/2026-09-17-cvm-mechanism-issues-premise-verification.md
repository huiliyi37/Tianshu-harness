---
title: 四条机制讨论 issue 的前提验证（#170–#173）
type: analysis
status: draft
date: 2026-09-17
related: []
...
## 维护者更正（2026-09-17，#170）

维护者在 #170 回复里指出一处前提更正，记在这里，免得本档案被当成替它背书：

> 页面上写的 cvm 报告实际上有一点出入。最开始的 AB 实验是为了信念宪法设立的。也就是有星域信念和无信念组来做对照。cvm 的产生反而在那之后。

即四条 issue 共同援引的那句「CVM 报告结尾说」并非该报告立论时的语境——**最早的 A/B 是「信念宪法」的有/无信念对照，CVM 是其之后才产生的**，因项目文档体量太大而被归并进 CVM 名下。

对本档案的影响：**不动任何一条已核验的事实**（本文档只核代码与行号，未对 CVM 的来龙去脉作过断言）。但四条那套共同 framing 里有一根引用建立在归并后的文档名上，援引它做论证时需知道这层出入——本文档原标题沿用 "CVM" 同属误标，已更正。

维护者同时确认：四篇设计**已收到**，将在周末细研；并指出设计落地之后还需从**日志轨迹**做观测。
