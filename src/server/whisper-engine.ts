 ...
 * spawn——POSIX 靠 shebang 能跑，Windows 会落到文件关联/EFTYPE（#149 同族）。
 * 命中时统一经 process.execPath 解释执行，于是「用 JS 包装脚本当 binPath」
 * 在两个平台都成立；真实 whisper-cli 二进制（无此后缀）不受影响。
 */
  ...
  return {
    transcribe(wavPath, o) {
      return new Promise((resolve, reject) => {
        ...
        // 后续 .stderr/.on 全部失型。
        const [cmd, cmdArgs] = JS_SCRIPT_RE.test(opts.binPath)
          ? [process.execPath, [opts.binPath, ...args]]
          : [opts.binPath, args]
